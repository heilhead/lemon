...
wgpu::Buffer gIndexBuffer;
wgpu::Buffer gVertexBuffer;

wgpu::Texture gTexture;
wgpu::Sampler gSampler;

wgpu::TextureView gDepthStencilView;
...
void initBuffers(const wgpu::Device& device) {
	...
	gIndexBuffer = CreateBufferFromData(device, indexData, sizeof(indexData), wgpu::BufferUsage::Index);

	static const float vertexData[12] = {
		0.0f, 0.5f, 0.0f, 1.0f, -0.5f, -0.5f, 0.0f, 1.0f, 0.5f, -0.5f, 0.0f, 1.0f,
	};

	gVertexBuffer = CreateBufferFromData(device, vertexData, sizeof(vertexData), wgpu::BufferUsage::Vertex);
}

void initTextures(const wgpu::Device& device, wgpu::Queue& queue) {
	wgpu::TextureDescriptor descriptor;
	descriptor.dimension = wgpu::TextureDimension::e2D;
	...
	gTexture = device.CreateTexture(&descriptor);

	gSampler = device.CreateSampler();
	...
	wgpu::ImageCopyTexture imageCopyTexture = CreateImageCopyTexture(gTexture, 0, { 0, 0, 0 });
...
void LemonEngine::listDevices() {
...

    {
        ...
        gDevice = wgpu::Device::Acquire(backendDevice);

        // stage 2

        gQueue = gDevice.GetQueue();

        auto swapChainImpl = dawn_native::d3d12::CreateNativeSwapChainImpl(backendDevice, wnd.getContextHandle());

        wgpu::SwapChainDescriptor swapChainDesc;
        swapChainDesc.implementation = reinterpret_cast<uint64_t>(&swapChainImpl);
        swapChain = gDevice.CreateSwapChain(nullptr, &swapChainDesc);
        auto textureFormat = static_cast<wgpu::TextureFormat>(dawn_native::d3d12::GetNativeSwapChainPreferredFormat(
            &swapChainImpl));
        swapChain.Configure(textureFormat, wgpu::TextureUsage::RenderAttachment, wndDesc.width, wndDesc.height);

        // stage 3

        initBuffers(gDevice);
        initTextures(gDevice, gQueue);

        // stage 4

        wgpu::ShaderModule vsModule = CreateShaderModule(gDevice, R"(
        [[stage(vertex)]] fn main([[location(0)]] pos : vec4<f32>)
                               -> [[builtin(position)]] vec4<f32> {
            return pos;
        })");

        wgpu::ShaderModule fsModule = CreateShaderModule(gDevice, R"(
        ...
        wgpu::PipelineLayout pl = MakeBasicPipelineLayout(gDevice, &bgl);

        gDepthStencilView = CreateDefaultDepthStencilView(gDevice);

        ComboRenderPipelineDescriptor descriptor;

        descriptor.layout = MakeBasicPipelineLayout(gDevice, &bgl);
        ...
        gPipeline = gDevice.CreateRenderPipeline((const wgpu::RenderPipelineDescriptor*)&descriptor);

        wgpu::TextureView view = gTexture.CreateView();

        gBindGroup = MakeBindGroup(gDevice, bgl, {{ 0, gSampler }, { 1, view }});
        ...
        wnd.loop([](auto) {
            ...
            ComboRenderPassDescriptor renderPass({ backbufferView }, gDepthStencilView);

            wgpu::CommandEncoder encoder = gDevice.CreateCommandEncoder();
            {
                wgpu::RenderPassEncoder pass = encoder.BeginRenderPass(&renderPass);
                pass.SetPipeline(gPipeline);
                pass.SetBindGroup(0, gBindGroup);
                pass.SetVertexBuffer(0, gVertexBuffer);
                pass.SetIndexBuffer(gIndexBuffer, wgpu::IndexFormat::Uint32);
                pass.DrawIndexed(3);
                pass.EndPass();
            }
