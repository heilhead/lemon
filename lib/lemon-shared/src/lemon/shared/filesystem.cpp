...
tl::expected<UnalignedMemory, Error>
lemon::io::readBinaryFile(const std::filesystem::path& filePath)
{
    return readFile<UnalignedMemory>(filePath,
                                     [](auto& is, auto size) -> tl::expected<UnalignedMemory, Error> {
                                         UnalignedMemory buffer(size);
